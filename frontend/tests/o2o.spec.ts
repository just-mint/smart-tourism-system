...
test.describe("O2O Domains Smoke Tests", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/api/v1/users/me", async (route) => {
      await route.fulfill({
        json: { id: 1, email: "admin@aegis.com", is_active: true, is_superuser: true, full_name: "Admin" }
      })
    })
  })
  ...
  test("Culture & Tourism page smoke test", async ({ page }) => {
    await page.route("**/api/v1/culture/places/search*", async (route) => {
      await route.fulfill({
        json: [
          { id: 1, name: "Hoan Kiem Lake", category: "Tourism", lat: 21.0285, lon: 105.8542 }
        ]
      })
    })

    await page.goto("/culture")
    await expect(page.getByText(/Văn hóa & Di sản/i)).toBeVisible({ timeout: 15000 })
    
    // Test search interaction
    await page.locator('input[data-testid="culture-search-input"]').fill("Hoan Kiem")
    await expect(page.getByText("Hoan Kiem Lake")).toBeVisible({ timeout: 10000 })
  })
  ...
  test("Spatial Operations page smoke test", async ({ page }) => {
    ...
    await expect(page.getByText(/Tìm thấy 1 địa điểm/i)).toBeVisible({ timeout: 10000 })
  })

  test("Inventory & Market page smoke test", async ({ page }) => {
    await page.route("**/api/v1/inventory/stores*", async (route) => {
      await route.fulfill({
        json: [{ store_id: 1, name: "Mock Store", address: "123 Street", lat: 21, lon: 105 }]
      })
    })
    await page.route("**/api/v1/inventory/stores/1/products*", async (route) => {
      await route.fulfill({
        json: [{ product_id: 101, name: "Mock Product", price: 100000, stock: 10 }]
      })
    })
  ...
  test("AI Vision Scan page smoke test", async ({ page }) => {
    ...
    await expect(page.getByText(/Vision & Closet/i)).toBeVisible({ timeout: 15000 })
  })

  test("Itinerary Planner page smoke test", async ({ page }) => {
    await page.goto("/itinerary")
    await expect(page.getByText(/SMART_PLANNER/i)).toBeVisible({ timeout: 15000 })
  })
  ...
  test("Checkout flow doesn't say 'paid' before payment", async ({ page }) => {
    ...
    await page.route("**/api/v1/inventory/stores/1/products*", async (route) => {
      await route.fulfill({ json: [{ product_id: 1, name: "Item X", price: 50000, stock: 5 }] })
    })
    await page.route("**/api/v1/inventory/lock*", async (route) => {
      await route.fulfill({ json: { message: "Locked", lock_id: 1, expires_at: new Date(Date.now() + 900000).toISOString() } })
    })
    ...
    await expect(page.getByText(/Order Created/i)).toBeVisible({ timeout: 10000 })
    await expect(page.getByText(/Scan with any banking app to complete payment/i)).toBeVisible()
