...
def _product_embedding_text(product) -> str:
    parts = [
        product.name,
        product.description,
        product.tags,
...
def _ensure_product_embeddings(db, limit: int = 1000) -> int:
    ...
    texts = [_product_embedding_text(product) for product in products]
    ...
    db.commit()
    logger.info(
        "[AI-Worker] Generated CLIP text embeddings for %s products", len(products)
    )
    return len(products)
