...
_GENERIC_IMAGE = "https://images.unsplash.com/photo-1549465220-1a8b9238cd48?auto=format&fit=crop&w=700&q=80"


def _product_image_url(product: Product) -> str | None:
    """Trả về ảnh phù hợp với tên sản phẩm. Ưu tiên keyword matching."""
    name = (product.name or "").lower()
    for keyword, url in PRODUCT_IMAGE_FALLBACKS:
        if keyword in name:
            return url
    # Nếu có image_url hợp lệ trong DB (không phải placeholder), dùng nó
