...
def add_to_closet(db: Session, user_id: str, image_path: str):
    ...
    new_item = VirtualCloset(
        user_id=user_id, image_path=image_path, vector_embedding=None
    )
    db.add(new_item)
    db.commit()
    db.refresh(new_item)
...
def find_similar_products_for_closet(
    db: Session, closet_item_id: int, user_id: str, top_n: int = 5
):
    """
    ...
    closet_item = (
        db.query(VirtualCloset).filter(VirtualCloset.id == closet_item_id).first()
    )
    if not closet_item:
