...
async def get_store_ai_story(store_id: int, db: Session = Depends(get_db)):
    ...
    if not result:
        raise HTTPException(
            status_code=404, detail="Không tìm thấy địa điểm gắn với cửa hàng"
        )
    return result
...
def update_review(
    ...
    return service.update_review(
        db=db, review_id=review_id, review_data=review, user=current_user
    )


@router.delete(
    "/reviews/{review_id}",
    response_model=schema.ReviewActionResponse,
    dependencies=[Depends(rate_limit(limit=20, window_seconds=60))],
)
def delete_review(
    review_id: int,
    ...
    return service.delete_review(db=db, review_id=review_id, user=current_user)
...
def report_review(
    ...
    return service.report_review(db=db, review_id=review_id)
