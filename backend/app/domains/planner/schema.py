...
class PlannerRequest(BaseModel):
    ...
    current_lat: float = Field(
        ..., ge=-90, le=90, description="Vĩ độ hiện tại của khách"
    )
    current_lon: float = Field(
        ..., ge=-180, le=180, description="Kinh độ hiện tại của khách"
    )
    radius: int = Field(
        default=2000, ge=500, le=20000, description="Bán kính tìm kiếm (mét)"
    )
    keywords: str = Field(
        default="",
        max_length=200,
...
class PlannerResponse(BaseModel):
    ...
    total_candidates: int = Field(
        default=0, description="Tổng số ứng viên thô trước khi lọc"
    )
