...
OPTIMIZATION_SERVICE_URL = os.getenv("OPTIMIZATION_SERVICE_URL", "http://optimization_service:8001/api/v1/optimize")
...
async def generate_smart_itinerary(
    ...
    from app.domains.planner.schema import ContextInfo
    # Mapping đơn giản mô tả slot
    time_desc = "Đang tối ưu theo thời gian thực"
    if 6 <= current_hour < 11: time_desc = "Buổi sáng — Ưu tiên khởi đầu năng lượng"
    elif 11 <= current_hour < 14: time_desc = "Giờ nghỉ trưa — Ưu tiên ẩm thực & trong nhà"
    elif 18 <= current_hour < 22: time_desc = "Buổi tối — Khám phá ẩm thực & giải trí"
